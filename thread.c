...
static int thread_func(void *data) {
	...
	while (!kthread_should_stop()) {
		printk(KERN_DEBUG "nas_thread: ticks = %d\n", nas_timer_ticks);
		/* ticks  < 0 : inactive
		 *       == 0 : just expired
		 *        > 0 : active
		 */
		...
		if (nas_timer_ticks >= 0) {
			...
			if (ret == EBUSY) {
				// hdd is busy
				printk(KERN_INFO "nas_thread: device busy\n");
				nas_timer_ticks = TIMER_TICKS; // reset ticks
				goto restart;

			} else if (ret==ENOENT || ret==ENOTBLK) {
				// hdd disappeared
				printk(KERN_INFO "nas_thread: disk/mount not found\n");
				nas_timer_ticks = -1;
				goto restart;

			} else if (ret != 0) {
				printk(KERN_ERR "nas_thread: unknow error(%d)\n", ret);
			}
		} else
			goto restart;
		...
		if (nas_timer_ticks == 0) {
			...
			printk(KERN_INFO "nas_thread: timer expired\n");

			bdev = blkdev_get_by_mountpoint(mntpt); // hold bdev
			if (IS_ERR(bdev)){
				printk(KERN_ERR "nas_thread: %s is not a mountpoint\n", mntpt);
				goto restart;
			}
			...
			// umount
			printk(KERN_INFO "nas_thread: unmount(%s)\n", mntpt);
			if (nas_unmount(mntpt) != 0) {
				printk(KERN_ERR "nas_thread: umount() failed, retry\n");
			...
			// eject
			kthread_ssleep(1);
			printk(KERN_INFO "nas_thread: eject(%s)\n", mntpt);
			if (ioctl_by_bdev(bdev, CDROMEJECT, 0) != 0) {
				printk(KERN_ERR "nas_thread: eject() failed\n");
			}

			// remove(optional)
			kthread_ssleep(1);
			printk(KERN_INFO "nas_thread: remove(%s)\n", dev_name(dev));
			if (remove_usb_device(dev) != 0) {
				printk(KERN_ERR "nas_thread: remove() failed\n");
			}
			put_device(dev); // release dev
			blkdev_put(bdev, 0); // release bdev
			...
			// pulldown gpio
			kthread_ssleep(2);
			printk(KERN_INFO "nas_thread: pulldown gpio(%d)\n", gpio_pin);
			set_gpio(gpio_pin, 0);
		}
	...
	restart:
		if (kthread_should_park()) {
			printk(KERN_DEBUG "nas_thread: parkme\n");
			kthread_parkme();
		}
		else
